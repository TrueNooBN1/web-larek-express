...
export const postOrder = (req: Request, res: Response) => {
    ...
    .then((findedItems)=>{
      if(findedItems.length != items.length){
        throw new Error("Can`t find items in db");
      }

      let nullPriceIndex = findedItems.findIndex(item=>item.price === null);
      if(nullPriceIndex !== -1)
        throw new BadRequestError(`Item has no price ${findedItems[nullPriceIndex]._id}`);
      ...
      if(totalPrice != order.total){
        ...
        throw new BadRequestError("totalPrice != price");
      }
