...
import {
  AUTH_ACCESS_TOKEN_EXPIRY, AUTH_REFRESH_TOKEN_EXPIRY, JWT_ACCES_KEY, JWT_REFRESH_KEY,
} from '../config';
...
export const generateTokens = (userId: string) => {
  const accessToken = jwt.sign(
    { _id: userId },
    String(JWT_ACCES_KEY),
    { expiresIn: ms(AUTH_ACCESS_TOKEN_EXPIRY) },
  );
  const refreshToken = jwt.sign(
    { _id: userId },
    String(JWT_REFRESH_KEY),
    { expiresIn: ms(AUTH_REFRESH_TOKEN_EXPIRY) },
  );
